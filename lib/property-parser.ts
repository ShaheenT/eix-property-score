...
type MarkedFacts = PropertyFacts & {
  [JSON_LD_SOURCE]?: symbol;
};

type MarkedEvidence = PropertyEvidence[] & {
  [JSON_LD_SOURCE]?: symbol;
};
...
function propertyTypePriority(value: JsonLdObject): number {
  ...
  if (types.includes('house')) return 25;
  if (types.includes('apartment')) return 25;
...
function getObjectProperty(object: JsonLdObject, key: string): JsonLdObject | null {
...
function addStringFact(
  ...
  if (facts[field] !== undefined) return;

  const cleaned = getStringValue(value);
  if (!cleaned) return;

  facts[field] = cleaned as never;
  evidence.push({ field, value: cleaned, source: 'json_ld' });
}
...
function addNumberFact(
  ...
  evidence.push({ field, value: String(parsed), source: 'json_ld' });
}
...
function addPriceFact(
  ...
  if (facts.askingPriceCents !== undefined) return;
...
function extractObjectFacts(jsonLd: JsonLdObject): {
  facts: Partial<PropertyFacts>;
  evidence: PropertyEvidence[];
} {
  ...
  if (address) {
    addStringFact(facts, evidence, 'address', address['streetAddress']);
    addStringFact(facts, evidence, 'suburb', address['addressLocality']);
    addStringFact(facts, evidence, 'province', address['addressRegion']);
    addStringFact(facts, evidence, 'postalCode', address['postalCode']);
  }
  ...
  const floorSize = getObjectProperty(jsonLd, 'floorSize');
  if (floorSize) {
    addNumberFact(facts, evidence, 'floorSizeM2', floorSize['value']);
  } else {
    addNumberFact(facts, evidence, 'floorSizeM2', jsonLd['floorSize']);
  }

  const lotSize = getObjectProperty(jsonLd, 'lotSize');
  if (lotSize) {
    addNumberFact(facts, evidence, 'landSizeM2', lotSize['value']);
  } else {
    addNumberFact(facts, evidence, 'landSizeM2', jsonLd['lotSize']);
  }

  const offers = getObjectProperty(jsonLd, 'offers');
  if (offers) addPriceFact(facts, evidence, offers['price']);
  addPriceFact(facts, evidence, jsonLd['price']);

  const propertyType =
    getStringValue(jsonLd['propertyType']) ??
    getStringValue(jsonLd['additionalType']);
  addStringFact(facts, evidence, 'propertyType', propertyType);
...
export function mergeFacts(
  ...
  for (const source of sources) {
    const markedSource = source as MarkedFacts;
    const sourceId = markedSource[JSON_LD_SOURCE];

    if (
      sourceId &&
      markedTarget[JSON_LD_SOURCE] &&
      markedTarget[JSON_LD_SOURCE] !== sourceId
    ) {
      continue;
    }

    if (sourceId && !markedTarget[JSON_LD_SOURCE]) {
      Object.defineProperty(markedTarget, JSON_LD_SOURCE, {
        value: sourceId,
        enumerable: false,
        configurable: true,
      });
...
export function mergeEvidence(
  ...
  const markedTarget = target as MarkedEvidence;
  const existingFields = new Set(target.map((item) => item.field));

  for (const source of sources) {
    const markedSource = source as MarkedEvidence;
    const sourceId = markedSource[JSON_LD_SOURCE];
...
function collectJsonLdObjects(value: unknown): JsonLdObject[] {
  ...
  if (Array.isArray(value)) {
    for (const item of value) objects.push(...collectJsonLdObjects(item));
    return objects;
  }
  ...
  if (Array.isArray(graph)) {
    for (const item of graph) objects.push(...collectJsonLdObjects(item));
  }

  for (const key of ['mainEntity', 'item', 'itemOffered']) {
    const nested = value[key];
    if (nested !== undefined) objects.push(...collectJsonLdObjects(nested));
  }
...
function choosePropertyObject(objects: JsonLdObject[]): JsonLdObject | null {
  ...
  const scored = objects.map((object, index) => ({
    object,
    index,
    score: propertyTypePriority(object),
  }));

  const highestScore = Math.max(...scored.map((item) => item.score));
  const highest = scored.filter((item) => item.score === highestScore);

  if (highest.length === 1) return highest[0].object;

  // Multiple equally plausible property entities are unsafe to merge.
  // Returning no facts forces the caller to treat the source as ambiguous.
  return null;
}

function extractJsonLdFacts(jsonLd: unknown): {
  facts: PropertyFacts;
  evidence: PropertyEvidence[];
} {
  ...
  if (!selected) {
    return {
      facts: {} as PropertyFacts,
      evidence: [],
    };
  }

  const result = extractObjectFacts(selected);
  const sourceId = Symbol('json-ld-block');

  const facts = result.facts as MarkedFacts;
  const evidence = result.evidence as MarkedEvidence;

  Object.defineProperty(facts, JSON_LD_SOURCE, {
    value: sourceId,
    enumerable: false,
    configurable: true,
  });

  Object.defineProperty(evidence, JSON_LD_SOURCE, {
    value: sourceId,
    enumerable: false,
    configurable: true,
  });

...
export async function processReport(submissionId: string) {
  const { data: submission, error: submissionError } = await supabaseAdmin
    ...
    .select('id, listing_url, status, goal, customer:customers(name, email)')
    .eq('id', submissionId)
    .single();
  ...
  if (reportError || !report) throw new Error('Report record not found');
  if (report.status === 'sent' || report.status === 'completed') return { status: report.status, reportId: report.id };
  if (report.status === 'processing') return { status: 'processing', reportId: report.id };
  ...
  if (!claimed) {
    const { data: current } = await supabaseAdmin.from('reports').select('status').eq('id', report.id).single();
    return { status: current?.status || 'processing', reportId: report.id };
  }
  ...
  try {
    ...
    const accessToken = report.access_token || randomUUID();
    const reportUrl = `${BASE_URL}/report/${report.id}?token=${encodeURIComponent(accessToken)}`;
    ...
    return { status: 'completed', reportId: report.id, reportUrl, customer: submission.customer };
  } catch (error) {
    await supabaseAdmin.from('reports').update({ status: 'failed', processed_at: new Date().toISOString() }).eq('id', report.id);
    throw error;
  }
}

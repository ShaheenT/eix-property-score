...
export async function processReport(submissionId: string) {
  ...
  const { data: queuedReport, error: reportError } = await supabaseAdmin
    .from('reports')
  ...
  try {
    ...
    if (extraction.status !== 'extracted') {
      throw new Error(`Property extraction failed: ${extraction.status}`);
    }
    ...
    // Delivery is deliberately not marked successful here. The report is only
    // marked sent by the delivery function after its email provider succeeds.
    return {
      status: 'completed',

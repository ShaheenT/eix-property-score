...
export function LeadForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.buyer_type === 'international' && !form.buyer_country) { toast({ title: 'Country required', description: 'Tell us where you are buying from.', variant: 'destructive' }); return; }
    setSubmitting(true);
    try {
      const res = await fetch('/api/checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, email, whatsapp: normalisedWhatsapp, listing_url: listingUrl, goal: form.goal, product: 'standard_report', buyer_type: form.buyer_type, buyer_country: form.buyer_country || null, buyer_purpose: form.buyer_purpose || null, buyer_budget: form.buyer_budget || null }) });

...
export default function SuccessPage() {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      attempts += 1;
      try {
        const response = await fetch(`/api/report/status?submission_id=${encodeURIComponent(submissionId)}`, {
          cache: 'no-store',
        });
        ...
        <div className="mt-8 rounded-2xl border border-white/10 bg-white/5 p-6 text-center">
          {reportUrl ? (
            <>
              ...
              <a href={reportUrl} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-teal-400 px-8 py-4 text-sm font-bold text-midnight-900 transition-all hover:brightness-110">
                View My Report

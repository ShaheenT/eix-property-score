      ...
      <div className="mx-auto max-w-6xl">
        ...
        {!isPro && (
          <section className="overflow-hidden rounded-[32px] border border-white/10 bg-gradient-to-br from-white/[0.07] via-white/[0.025] to-teal-300/[0.04] p-6 shadow-2xl shadow-black/20 sm:p-9">
            ...
            <div className="mt-8 rounded-3xl border border-amber-200/15 bg-amber-200/[0.045] p-6 sm:p-7">
              ...
              <p className="mt-4 max-w-4xl text-sm leading-7 text-white/70">{decisionBody}</p>
              <div className="mt-6 grid gap-3 sm:grid-cols-4">
                <SignalCard label="Price" value={price} detail="The transaction's biggest unresolved variable." tone="attention" />
                <SignalCard label="Physical proposition" value={`${text(facts.bedrooms)} bed · ${text(facts.bathrooms)} bath`} detail={floorM2 !== null ? `${number(floorM2)} m² internal` : 'Size not fully established'} tone="positive" />
                <SignalCard label="Market evidence" value={comparableCount > 0 ? `${comparableCount} verified` : 'Not enough'} detail="No market conclusion is manufactured." tone={comparableCount > 0 ? 'positive' : 'attention'} />
                <SignalCard label="Next action" value="Challenge the price" detail="Build the comparable case before an offer." />
              </div>
            </div>
          </section>
        )}

...
export async function POST(req: NextRequest) {
  try {
    ...
    const { url, reference } = await initializePaystackTransaction({ amount, itemName, submissionId: submission.id, paymentId: payment.id, customerEmail: customer.email, customerName: customer.name, product: prod });

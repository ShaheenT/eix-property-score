...
export async function GET(req: NextRequest) {
  ...
  try {
    const { data: submission, error: submissionError } = await supabaseAdmin
      .from('property_submissions').select('id, status').eq('id', submissionId).single();
    if (submissionError || !submission) return NextResponse.json({ error: 'Submission not found' }, { status: 404 });
    if (submission.status !== 'paid') return NextResponse.json({ status: 'awaiting_payment' });

    const { data: report, error: reportError } = await supabaseAdmin
      .from('reports').select('id, status, access_token').eq('submission_id', submissionId).maybeSingle();
    if (reportError) throw reportError;
    if (!report) return NextResponse.json({ status: 'queued' });

    if (report.status === 'completed' || report.status === 'sent') {
      const reportUrl = `${process.env.NEXT_PUBLIC_BASE_URL || 'https://eix-property-score-beta.vercel.app'}/report/${report.id}?token=${encodeURIComponent(report.access_token || '')}`;
    ...
    if (report.status === 'failed' || report.status === 'queued') {
      ...
      if (report.status === 'failed') return NextResponse.json({ status: 'failed' }, { status: 500 });
    }
